        ...
        <section v-show="step === 2" class="rw-panel">
          <div class="rw-card">
            ...
            <div class="rw-card-body">
              ...
              <!-- Медицинские сведения и реабилитационная группа -->
              ...
              <div class="rw-fg">
                ...
                <!-- ЦРГ — кастомный single-select (как в макете) -->
                <div class="rw-f rw-c12">
        ...
        <section v-show="step === 3" class="rw-panel">
          <div class="rw-card">
            ...
            <div class="rw-card-body">
              ...
              <!-- Subsection 1: Сканы готовых документов -->
              ...
              <!-- Subsection 2: Документы на подпись -->
              ...
              <!-- Subsection 3: Подписанные документы -->
              <div class="rw-subsection">
                <div class="rw-ssh">
...
const f = ref({
  ...
  lrAddress: '',
  // Step 2 — ФИО и дата рождения
  rLast: '', rFirst: '', rMid: '',
  rBirth: '',
  // Step 2 — Медицинские сведения
  rInvalidity: '',
  rSnils: '',
  rMseDate: '',
  rMseUntil: '',
  ...
  // Step 2 — Адрес регистрации
  rRegOkrug: '', rAddrReg: '',
  // Step 2 — Фактическое проживание
  rAddrSame: false,
  rFactOkrug: '', rAddrFact: '',
  // Step 2 — Место обучения
  rEduName: '',
  // Step 2 — Особенности
  rSpecial: '',
  // Step 3
...
// Must be after f is defined — these reference f.value
...
// Сброс ЦРГ при смене возрастной категории
...
// Сбрасываем подгруппу при смене группы
...
const save = async () => {
  ...
  saving.value = true;
  try {
    const fullName = [f.value.rLast, f.value.rFirst, f.value.rMid].filter(Boolean).join(' ');
    const lrName  = [f.value.lrLast, f.value.lrFirst, f.value.lrMid].filter(Boolean).join(' ');
    let age = null;
    if (f.value.rBirth) {
      age = new Date().getFullYear() - new Date(f.value.rBirth).getFullYear();
    }
    await api.post('/recipients', {
      ...
      photo:               '',
      contacts:            f.value.lrPhone,
      legalRepresentative: lrName,
      aggressionLevel:     1,
      showAggression:      false,
      aggressionNote:      f.value.rSpecial || '',
