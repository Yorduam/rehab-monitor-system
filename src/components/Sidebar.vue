<template>
  ...
  <!-- ═══════════════════════════════════════════════════
       TEACHER / CURATOR SIDEBAR  — дизайн по макету v2
  ═══════════════════════════════════════════════════ -->
  <aside
  ...
  >
    ...
    <nav class="s-nav">
      ...
      >
        ...
        Реабилитанты
        <span v-if="recipientsCount" class="s-badge">{{ recipientsCount }}</span>
      </button>
      ...
      <button
        class="s-item"
        :class="{ 's-item--active': pageStore.current === 'timeline' }"
        @click="go('timeline', 'Расписание')"
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    <nav class="s-nav">
      ...
      <button
        class="s-item"
        :class="{ 's-item--active': pageStore.current === 'programs' }"
        @click="go('programs', 'Программы')"
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
          <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/>
          <polyline points="14 2 14 8 20 8"/>
    ...
    <div class="s-footer">
      <div class="s-user">
        ...
        <div class="s-user-text">
          <div class="s-user-name">{{ authStore.user?.fullName || 'Пользователь' }}</div>
  ...
  <!-- ═══════════════════════════════════════════════════
       ADMIN / OTHER ROLES SIDEBAR — существующий дизайн
  ═══════════════════════════════════════════════════ -->
  <aside v-else class="sidebar" aria-label="Основная навигация">
    ...
    <div class="sidebar-footer">
      <div class="user-info">
        <div class="avatar">{{ authStore.user?.fullName?.charAt(0) || 'А' }}</div>
        <div>
          <div class="name">{{ authStore.user?.fullName || 'Пользователь' }}</div>
...
const userInitials = computed(() => {
  ...
  return name.trim().split(/\s+/).map(w => w[0]).filter(Boolean).slice(0, 2).join('').toUpperCase() || 'П';
});
...
// Обновляем CSS-переменную, чтобы .main в App.vue получил правильный margin-left
const updateSidebarWidth = (isTeacher) => {
  document.documentElement.style.setProperty(
    '--sidebar-width',
    isTeacher ? '240px' : '260px'
  );
};
...
const analyticsNav = [
  ...
  { id: 'programs',   label: 'Программы',   icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>' },
  ...
  { id: 'timeline',   label: 'Таймлайн',    icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>' }
];
...
/* ══════════════════════════════════════════════════════════════
   TEACHER SIDEBAR
   Все стили под .sidebar-teacher — никаких конфликтов с .sidebar
══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   ADMIN / OTHER ROLES SIDEBAR — существующие стили (без изменений)
══════════════════════════════════════════════════════════════ */
