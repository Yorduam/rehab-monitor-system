  ...
  <div>
    ...
    <div v-else class="items-grid">
      <div v-for="g in groups" :key="g.id" class="item-card" @click="openDetailsModal(g)">
        ...
        <div class="tags">
          <span class="badge badge-blue">{{ g.direction }}</span>
    ...
    <Modal v-if="modalVisible" :title="modalTitle" @close="modalVisible = false">
      <form @submit.prevent="saveGroup">
        ...
        <div class="form-group">
          ...
          <input v-model="form.direction" class="form-input" />
        </div>
    ...
    <!-- Модальное окно деталей группы (куратор + участники) -->
    <Modal v-if="detailsModalVisible" :title="`Группа: ${selectedGroup?.name}`" @close="detailsModalVisible = false">
      <div class="group-details">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ selectedGroup?.direction || '—' }}</span>
        </div>
        <div class="detail-row">
          ...
          <div class="participants-list">
            ...
            <div v-else v-for="p in participants" :key="p.id" class="participant-item">
              ...
              <div class="participant-info">
                ...
                <div class="participant-meta">{{ p.age }} лет · {{ p.diagnosis || '—' }}</div>
              </div>
            </div>
          </div>
        </div>
...
const form = ref({ name: '', curatorId: '', direction: '' })
...
// Открыть модальное окно деталей группы
...
const openAddModal = () => {
  editId.value = null
  form.value = { name: '', curatorId: curators.value[0]?.id || '', direction: '' }
...
<style scoped>
/* Ваши стили остаются, добавляем новые для деталей группы */
.group-details {
  display: flex;
