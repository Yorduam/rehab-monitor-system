...
const stats = ref({
  recipients: 0, activeCount: 0, pendingCount: 0, completedCount: 0,
...
const donutSegments = computed(() => {
  ...
  const raw = [
    ...
    { value: pending,   color: '#B07223' },
    { value: completed, color: '#D6CFBE' }
  ].filter(s => s.value > 0);

  if (raw.length === 0) return [];

  const effectiveC      = C - raw.length * GAP;
  const INITIAL_OFFSET  = -C / 4; // starts at 12 o'clock with rotate(-90deg)
...
const kpiRows = computed(() => {
  ...
  return [
    ...
    { color: '#B07223', label: 'Ожидает начала',      value: stats.value.pendingCount   || 0, pct: Math.round(((stats.value.pendingCount   || 0) / total) * 100) },
    { color: '#D6CFBE', label: 'Завершили программу', value: stats.value.completedCount || 0, pct: Math.round(((stats.value.completedCount || 0) / total) * 100) }
  ];
});
...
/* ── lifecycle ────────────────────────────────────────────── */
onMounted(() => {
  // Warm off-white background matching ERP-R canvas
