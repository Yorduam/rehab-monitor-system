  ...
  <div class="diagnostics-page">
    ...
    <div class="content">
      ...
      <div class="grid">
        ...
        <!-- ======== ОСНОВНАЯ КОЛОНКА — ЭТАПЫ ======== -->
        <div id="stages-flow">

          <!-- ============================================ -->
          <!-- ЭТАП 01: ПСИХОЛОГ (свёрнут, завершён)     -->
          <!-- ============================================ -->
          ...
          <!-- ============================================ -->
          <!-- ============================================ -->
          <!-- ЭТАП 02: АФК (раскрыт, в процессе)        -->
          <!-- ============================================ -->
          <article class="stage-card" id="stage-afk" data-stage="afk" data-status="progress">
            ...
            <div class="stage-body" id="stage-afk-body">
              ...
              <!-- Комментарий специалиста по АФК -->
              <fieldset class="qgroup">
                <legend class="qlabel">
                  <span class="qlabel-text">Комментарий специалиста</span>
                </legend>
          ...
          <!-- ============================================ -->
          <!-- ЭТАП 03: СОЦ.-КУЛЬТУРНАЯ (раскрыт, активный) -->
          <!-- ============================================ -->
...
onMounted(() => {
  document.title = 'ERP-Р • Диагностика — Мария Петрова'
    ...
    // ============ ВЫБОР РЕАБИЛИТАНТА ПО ДВОЙНОМУ КЛИКУ ============
    ...
    function normalizeRecipient(raw, idx = 0) {
      const fullName = normalizeSpaces(raw.fullName || raw.name || raw.fio || raw.title || 'Реабилитант');
      const id = raw.id ?? raw.recipientId ?? (idx + 1);
      const age = raw.age ?? raw.years ?? '';
      const birthDateLabel = raw.birthDateLabel || raw.birthDate || raw.dateOfBirth || raw.birthday || '';
      const diagnosis = normalizeSpaces(raw.diagnosis || raw.nosology || raw.program || 'Нозология не указана');
      const groupName = normalizeSpaces(raw.groupName || raw.group?.name || raw.cgr || raw.category || 'Группа не указана');
      ...
      return {
        ...
        age,
        birthDateLabel: birthDateLabel ? formatDateRu(birthDateLabel) : '',
        diagnosis,
        groupName,
        code,
    ...
    async function loadRecipientsFromProject() {
      try {
        ...
        // В офлайн-режиме или без авторизации оставляем локальный список.
...
// ============ STAGE TILE → открыть этап ============
    ...
    // ============ SEGMENTED ============
    // Семантика цвета через data-scheme:
    //   "rating"      → 1-й = sage, 2-й = amber, 3-й = rose (3 элемента);
    //                   1-й = sage, 2-й = rose (2 элемента);
    //                   4 элемента: sage, sage, amber, rose
    //   "rating-asc"  → возрастание-хорошо (Низкий → Высокий): 1-й rose, 2-й amber, 3-й sage
    //   "tonus"       → норма посередине, отклонения по краям: amber, sage, amber
    //   без data-scheme → активный сегмент всегда sage
    ...
    // ============ TRIPLE CONTROL (АФК тесты + эмоц-волевая сфера) ============
    // Цветовая схема каждой кнопки определяется индивидуально:
    //   - если у кнопки задан цветовой класс в HTML (yes / partial / no),
    //     этот класс будет применяться при активации именно этой кнопки;
    //   - иначе — fallback по позиции в группе: 0=yes, 1=partial, 2=no.
    document.querySelectorAll('.triple-control').forEach(group => {
      ...
      // Запоминаем «персональный» цвет каждой кнопки один раз при инициализации
    ...
    // ============ GMFCS / LEVEL CARDS / THEATRE OPTIONS ============
    ...
    // GMFCS: при выборе .none — родительский grid получает is-disabled,
    // и уровни визуально заглушаются.
    (function() {
    ...
    radioCards('.level-card', [null, null, null]); // цвет хранится в HTML
    // театр: 3 опции на ряд (low/mid/high)
    ...
    // ============ FINISH STAGE ============
    // При нажатии «Завершить этап» — этап получает статус "done":
    // плитка маршрута, side-nav-пункт и бейдж в шапке этапа обновляются,
    // счётчик завершённых этапов растёт, кнопка превращается в «Отредактировать этап».
    (function() {
      // Запомненные исходные DOM-снапшоты для восстановления при «Отредактировать»
      ...
      function smoothCollapse(card, callback) {
        ...
        const onEnd = () => {
          ...
          // компенсация скролла: после сворачивания страница "уезжает" — подталкиваем обратно
        ...
        // запасной таймер, если transitionend не прилетит
      ...
      function smoothExpand(card) {
        ...
        // лёгкая slide-in анимация — уже есть через keyframes bodyIn
      }
      ...
      function setStageStatus(stageKey, newStatus, opts) {
        ...
        // номер этапа: при done — галочка, при revert — обратно цифра
        ...
        // sub-status в табах внутри этапа (для соц-культурной)
        if (newStatus === 'done') {
      ...
      // ===== Блокировка полей завершённого этапа =====
      // Подсвечивает .stage-body как read-only и проставляет disabled/readonly
      // на все интерактивные элементы. Запоминает в data-locked-prev предыдущее
      // состояние, чтобы корректно восстановить при разблокировке.
      function lockStageBody(card) {
        ...
        // Все интерактивные кнопки внутри полей (не относящиеся к .stage-actions)
        ...
        // Текстовые поля делаем readonly (не disabled — чтобы сохранить читаемость)
        ...
        // Баннер «Этап завершён» в начале панели
      ...
      function bindFinishButtons() {
        document.querySelectorAll('[data-action="finish-stage"]').forEach(btn => {
          // Только проверяем dataset.bound. НЕ проверяем btn.disabled, потому что
          // кнопка может быть disabled на старте (если не все подблоки завершены)
          // и стать enabled позже — обработчик нужен в любом случае.
          if (btn.dataset.bound === '1') return;
          btn.dataset.bound = '1';
          btn.addEventListener('click', () => {
            // На клик: если кнопка disabled — игнорируем
            ...
            // блокируем поля этапа
            lockStageBody(card);
            // пульс и подмена кнопки
            card.classList.add('just-finished');
            setTimeout(() => card.classList.remove('just-finished'), 800);
            replaceFinishButton(card, 'edit');
            // плавное сворачивание + скролл к следующему этапу
            setTimeout(() => {
              smoothCollapse(card, () => {
                // ищем следующий ещё открытый/незавершённый этап
                ...
                // если следующего нет — оставляем фокус на текущей завершённой шапке
                const scrollTarget = target || card;
                // используем scroll-margin-top через CSS-переменную --stick-offset
    ...
    (function() {
      document.querySelectorAll('[data-action="save-draft"]').forEach(btn => {
        btn.addEventListener('click', () => {
          // визуальный feedback: галочка и текст «Сохранено»
          ...
          // обновим индикатор в топбаре, если есть
          setTimeout(() => {
    ...
    // ============ FINISH SUB-BLOCK (универсально для всех этапов с подтабами) ============
    (function() {
      // Блокирует поля подпанели и расставляет data-locked-prev,
      // чтобы при разблокировке корректно восстановить состояние.
      function lockSubpanel(panel) {
        ...
        interactiveBtns.forEach(b => {
          // не блокируем кнопки .subblock-actions
        ...
        // Баннер «Блок завершён» сразу после subpanel-head
      ...
      // Считает завершённость всех подпанелей этапа и активирует/деактивирует
      // кнопку «Завершить этап» в самом низу этапа.
      function updateStageReadiness(stageCard) {
        if (!stageCard) return;
        const subpanels = stageCard.querySelectorAll(':scope > .stage-body > .subpanel');
        if (!subpanels.length) return; // у этапа нет подразделов
        ...
        if (hint) {
          if (allDone) {
            ...
            // обновляем текст: "Осталось N из M подразделов"
            const text = remaining === 1
              ? 'Остался последний подраздел'
              : 'Осталось ' + remaining + ' из ' + total + ' подразделов';
            // первый Element.textContent в свету: меняем только текстовый узел после svg
            ...
            // если текстового узла не нашли — добавляем
            hint.appendChild(document.createTextNode(' ' + text));
          }
        }
      }

      // Меняет кнопку «Завершить блок» ↔ «Отредактировать блок» (через outerHTML,
      // затем пере-привязываем обработчики).
      function rebindActions(card) {
        // переподписки кнопок этапа (finish-stage / edit-stage) делает существующий
        // bindFinishButtons; вызовем его на всякий случай.
      ...
      function attachSubblockBtnHandlers() {
        ...
        document.querySelectorAll('[data-action="edit-subblock"]').forEach(btn => {
          ...
          btn.addEventListener('click', () => {
            ...
            if (!panel || !stageCard) return;
            const key = panel.id.replace('subpanel-', '');
            // sub-status снова в "in progress"
            const dot = stageCard.querySelector('.subtab[data-subtab="' + key + '"] .sub-status');
            if (dot) {
      ...
      attachSubblockBtnHandlers();
      // первичная проверка готовности всех этапов с подпанелями
      document.querySelectorAll('.stage-card').forEach(updateStageReadiness);

      // экспортируем, если потребуется снаружи
    ...
    // ============ ЗАВЕРШЕНИЕ ДИАГНОСТИКИ + PDF-ОТЧЁТ ============
    (function() {
      ...
      function collectReportData() {
        ...
        // Возвращает массив ФИО специалистов, добавленных через UI (только .specialist-chip).
        ...
        // Возвращает только текст из textarea-полей панели (рекомендации специалиста).
        ...
        // То же, но ищет textarea внутри stage-card[data-stage="..."].
      ...
      function drawReportCanvases(data) {
        ...
        const HEADER_H = 67;   // высота строки-заголовка таблицы
        const TABLE_TOP = 462; // Y верхней границы таблицы
        const ROW_H = 100;     // высота одной строки специалиста

        // Вычисляем количество строк и высоту каждого блока динамически
        const blocks = (data.blocks || []).map(b => {
          const rowCount = Math.max(1, (b.specialists || []).length);
          return Object.assign({}, b, { rowCount, h: rowCount * ROW_H });
        });

        // Назначаем Y-координаты блоков последовательно
        ...
        // Создаём canvas нужной высоты (минимум 2000px)
        ...
        // ── Колонка «Направление» (col-0): вертикальный текст по группам ──
        if (psyBlock) {
        ...
        if (culBlocks.length) {
          ...
          // Разделители между поднаправлениями (начиная от col-1, не разрезая col-0)
          culBlocks.slice(0, -1).forEach(b =>
            drawLine(ctx, tableX[1], b.y2, tableX[6], b.y2, 2.0)
          );
        }
        if (afkBlock) {
          // АФК: cols 0+1 объединены, горизонтальный текст
        ...
        blocks.forEach(b => {
          ...
          // Поднаправление (col-1, левый верхний угол блока, только не-АФК)
          if (b.sub) {
          ...
          // Рекомендации (col-2): только текст из textarea, абзацы через пустую строку
          if (b.recs && b.recs.trim()) {
            ...
            b.recs.split('\n').forEach((para, pi) => {
              if (!para.trim()) return;
              if (pi > 0 && allLines.length) allLines.push(''); // отступ между абзацами
          ...
          // Должность (col-3): центрируется по всей высоте блока
          drawCenteredBlock(ctx, b.pos, tableX[3], b.y1, tableX[4], b.y2, { font: '22px ' + serif, lineHeight: 26 });

          // ФИО + подпись (cols 4–5): каждый специалист — отдельная строка ROW_H
          ...
          names.forEach((name, si) => {
            ...
            // Тонкая разделительная линия между строками ФИО (не после последней)
        ...
        // ── Блок ознакомления и подписи ───────────────────────
        // Все смещения считаются от tableBottom, чтобы footer следовал за таблицей.
    ...
    // ============ СУММЫ БАЛЛОВ В СУБПАНЕЛЯХ ============
    (function() {
      // ИЗО: сумма по point-btn активным (value = индекс+1 → но у нас классы high/mid/low)
      // удобнее использовать data-val
      ...
      // ВОКАЛ: сумма scale-value по 5 шкалам
    ...
    // ============ STICKY ENGINE ============
    // Управляет: 1) sticky-hero (с гистерезисом против дребезга),
    //            2) CSS-переменной --stick-offset (низ топбара+hero),
    //               --stick-offset-inner (низ ... + шапка соц-этапа),
    //               --stick-offset-subhead (низ ... + subtabs),
    //            3) detect стек-состояний (head-stuck / subtabs-stuck / subhead-stuck) для теней,
    //            4) scroll-spy: подсветка активного этапа в side-nav.
    (function() {
      ...
      // ===== Кэш высот: пересчитываются только когда меняются (ResizeObserver),
      //       а не на каждом кадре скролла =====
      let topbarH = 0, heroH = 0, socHeadH = 0, socSubtabsH = 0;
      let stickOffset = 0, stickOffsetInner = 0, stickOffsetSubhead = 0;

      // Активный соц-культ этап (где есть подтабы)
      ...
      // Кэш .stage-head-clickable для каждой карточки (один раз на init)
      ...
      function recomputeHeights() {
        ...
        // Если базовый offset существенно изменился — пересоздаём scroll-spy observer
        // (его rootMargin привязан к stickOffset)
        if (Math.abs(stickOffset - prevStickOffset) > 4) {
          buildSpyObserver();
        }
      }

      // ===== Hero sticky: через IntersectionObserver, а не через rect на каждом кадре =====
      let heroStuck = false;
      if ('IntersectionObserver' in window && heroSentinel) {
        const io = new IntersectionObserver((entries) => {
          for (const entry of entries) {
            // sentinel «выехал» вверх за пределы видимой области — hero прилип
      ...
      // ===== Состояния «прилипло» для шапки этапа / субтабов / subhead =====
      // Считаем только для карточек, попавших в наблюдаемый диапазон возле sticky-стека
      let activePanel = null;
      let activeSubhead = null;

      function updateStuckStates() {
        // Все getBoundingClientRect делаем единым «батчем» (один reflow)
      ...
      // ===== Scroll-spy через IntersectionObserver вместо getBoundingClientRect-loop =====
      // Идея: сужаем «активную зону» через rootMargin до тонкой горизонтальной полоски
      // прямо под sticky-стеком (topbar + hero). Активной считаем самую верхнюю карточку,
      // пересекающую эту полоску — она и есть то, что пользователь сейчас читает.
      const sectionNameEl = document.getElementById('hero-section-name');
      const stageNameCache = new Map(); // data-stage -> displayName
      ...
      function pickActiveStage() {
        // Среди карточек, пересекающих «активную полоску», выбираем самую верхнюю
        // (с минимальным top относительно вьюпорта). Если ничего не пересекает —
        // используем последнюю «прокрученную» (если скроллили вниз) или первую.
        let bestKey = null;
        ...
        // Fallback: если в полоске никого — берём ту, чей низ ближе всего сверху над полоской
        if (!bestKey) {
      ...
      function buildSpyObserver() {
        ...
        // Сужаем root до тонкой полоски сразу под sticky-hero.
        // Сверху отрезаем stickOffset, снизу — почти всю высоту окна, оставляя ~100px зону.
      ...
      // ===== Лёгкий scroll-обработчик: только sticky-тени (всё остальное — observers) =====
      let ticking = false;
      function tick() {
        ticking = false;
        updateStuckStates();
      ...
      // ===== Resize/изменения геометрии: пересчитываем кэш высот один раз =====
      if ('ResizeObserver' in window) {
        const ro = new ResizeObserver(() => {
          recomputeHeights();
          // после изменения размеров — однократно пересчитаем sticky-состояния
          if (!ticking) {
            requestAnimationFrame(tick);
            ticking = true;
          }
      ...
      // Изменение состава активной субпанели — обновляем кэш
      if (socCard) {
      ...
      // Изменение collapsed-состояния этапов — пересчёт высот
      ...
      requestAnimationFrame(() => {
        recomputeHeights();
        buildSpyObserver();
        // Принудительно выбираем активный этап на старте, чтобы подпись
        // в sticky-hero показывала правильный раздел сразу
        pickActiveStage();
        requestAnimationFrame(tick);
      });
    })();
    ...
    (function() {
      ...
      function getAlreadyAdded(addBtn) {
        ...
        row.querySelectorAll('.specialist-chip').forEach(chip => {
          // используем initials как маркер совпадения
...
.diagnostics-page{
      ...
      /* Высота sticky-стека сверху (топбар + hero). Обновляется JS-ом. */
      --stick-offset: 10rem;
    ...
    .diagnostics-page .topbar{
      ...
      top: 0;
      z-index: 40;
      /* непрозрачный фон вместо backdrop-filter — на слабых машинах
         blur на каждом кадре скролла очень дорог */
      background: var(--paper);
    ...
    .diagnostics-page .hero{
      ...
      align-items: center;
      position: sticky;
      top: var(--project-topbar-h, 3.75rem); /* высота топбара проекта */
      z-index: 35;
      /* transition оставлен только для box-shadow — padding и border-radius
         триггерят layout/reflow при «прилипании» hero */
    ...
    .diagnostics-page .hero.is-stuck .hero-avatar{
      ...
      flex: 0 0 2.5rem;
      /* transition убран: смена width/height/font-size триггерит layout
         на каждом кадре всей страницы, это очень дорого */
    }
    ...
    /* Подпись «Раздел: …» — видна только в sticky-режиме */
    ...
    .diagnostics-page .hero-avatar{
      ...
      flex: 0 0 4.5rem;
      /* без transition: width/height/font-size — это вызывало
         layout/reflow всей страницы при каждом «прилипании» hero */
    }
    ...
    /* «текущий» этап в маршруте — без выделения; статус передаётся цветом фона */
    ...
    .diagnostics-page .stage-card{
      ...
      /* content-visibility пропускает рендеринг внутренностей карточки,
         пока она вне видимой области — критично для длинной страницы */
    ...
    .diagnostics-page .stage-card.just-finished{
      /* убираем «дорогую» анимацию box-shadow — вместо неё используем
         псевдо-элемент с opacity+scale, который композитится на GPU */
      position: relative;
    }
    ...
    /* ============ ЗАБЛОКИРОВАННЫЙ ЭТАП (после "Завершить этап") ============
       Поля показаны как read-only. Полноценная блокировка обеспечивается:
       - в JS отдельно: button.disabled и input.disabled / readonly,
       - здесь: pointer-events на поля и визуальное приглушение. */
    ...
    .diagnostics-page .stage-body.is-locked select{
      pointer-events: none;
      user-select: text;        /* читать можно, выделять текст можно */
    }
    /* Приглушение всех интерактивных полей */
    ...
    /* Неактивные элементы приглушаем сильнее, выбранные — оставляем читаемыми */
    ...
    /* Текстовые поля становятся «read-only» визуально */
    .diagnostics-page .stage-body.is-locked textarea,
    .diagnostics-page .stage-body.is-locked input[type="text"],
    .diagnostics-page .stage-body.is-locked input[type="number"],
    .diagnostics-page .stage-body.is-locked input[type="search"]{
    ...
    /* Кнопки внутри панели — отключены (но кнопка «Отредактировать этап»
       находится в .stage-actions, а её мы не отключаем) */
    ...
    /* Плашка-индикатор «Этап завершён» сверху панели */
    ...
    .diagnostics-page .stage-toggle{
      display: contents;     /* передаём весь head как кликабельную область */
    }
    ...
    /* ============ ЗАБЛОКИРОВАННЫЙ ПОДБЛОК ============
       Подблок завершён: поля внутри read-only, видна плашка-индикатор,
       кнопка «Отредактировать блок» остаётся активной. */
    ...
    /* Кнопки внутри блока: курсор по умолчанию */
    ...
    /* Действия подблока — остаются активными, чтобы можно было нажать «Отредактировать блок» */
    .diagnostics-page .subpanel.is-locked .subblock-actions{ pointer-events: auto; }

    /* Плашка «Блок завершён» в начале подблока */
    ...
    /* ============ КНОПКА «ЗАВЕРШИТЬ ЭТАП» в состоянии ожидания подблоков ============ */
    ...
    .diagnostics-page .save-bar{
      ...
      right: 0;
      z-index: 50;
      /* приглушённый серый фон в тон palette; непрозрачный, без backdrop-filter */
      background: var(--paper-sunken);
    ...
    /* Вариант test-row, где справа располагается chip-group (много вариантов),
       не умещающийся в одну колонку. chip-group переносится на новую строку
       под описанием на всю ширину. */
    ...
    .diagnostics-page .triple-control{
      ...
      border-radius: 0.5rem;
      padding: 0.1875rem;
      /* Единая ширина для всех вопросов — независимо от числа кнопок (2 или 3),
         блок занимает одинаковое место, кнопки делят ширину поровну.
         Длинные подписи переносятся в 2 строки, не обрезаясь. */
      width: 26rem;
      flex: 0 0 26rem;
    }
    .diagnostics-page .triple-btn{
      ...
      gap: 0.3125rem;
      transition: background 0.15s, color 0.15s;
      /* допускаем перенос длинных подписей вместо обрезки */
      white-space: normal;
      text-align: center;
      line-height: 1.2;
    ...
    .diagnostics-page .gmfcs-grid.is-disabled .gmfcs-card:not(.none){
      background: var(--paper-sunken);
      opacity: 0.45;
      /* filter: grayscale убран — он создаёт layer и тормозит при hover.
         Снижение opacity даёт похожий эффект «приглушения» дешевле */
      border-color: var(--line);
      cursor: pointer;
    }
    ...
    /* ============ ОЦЕНКА УРОВНЯ (1-2-3) для общего вывода АФК ============ */
    ...
    /* ============ ИЗО — таблица критериев с 3 баллами ============ */
