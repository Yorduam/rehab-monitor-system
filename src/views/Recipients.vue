<template>
  <div>
    ...
    <!-- ═══════════════════════════════════════════════
         TEACHER / CURATOR VIEW  (новый дизайн по макету)
    ═══════════════════════════════════════════════ -->
      ...
      <div class="erp-r-teacher" @click="closeDropdown">
        ...
        <div class="t-page-header">
          <div>
            ...
            <p class="t-page-sub">
              Всего <strong>{{ totalCount || recipients.length }}</strong> активных
              <template v-if="attentionCount"> · требуют внимания — <strong>{{ attentionCount }}</strong></template>
            </p>
          </div>
        ...
        <div class="t-filter-row" role="group" aria-label="Быстрые фильтры">
          ...
          <button class="t-chip" :class="{ active: activeFilter === 'attention' }" :aria-pressed="activeFilter === 'attention'" @click="setFilter('attention')">
            Требуют внимания <span class="t-chip-count">{{ attentionCount }}</span>
          </button>
          <button class="t-chip" :class="{ active: activeFilter === 'in_progress' }" :aria-pressed="activeFilter === 'in_progress'" @click="setFilter('in_progress')">
            В процессе <span class="t-chip-count">{{ inProgressCount }}</span>
          </button>
          <button class="t-chip" :class="{ active: activeFilter === 'completed' }" :aria-pressed="activeFilter === 'completed'" @click="setFilter('completed')">
            Завершили <span class="t-chip-count">{{ completedCount }}</span>
          </button>
        </div>
        ...
        <div v-else-if="viewMode === 'grid'" class="t-card-grid" role="list" aria-label="Список реабилитантов">
          <article
            ...
            class="t-rcard"
            :class="{
              'has-alert': hasAlert(r),
              'completed-card': r.completionStatus === 'completed',
          ...
          >
            ...
            <!-- Alert flags (top-right) -->
            <div v-if="hasAlert(r)" class="t-rcard-flags" role="group" aria-label="Флаги внимания">
              <span
                v-if="r.showAggression && r.aggressionLevel >= 3"
                class="t-flag t-flag-rose"
                :title="r.aggressionNote || 'Особенность поведения: агрессия'"
                role="img"
                :aria-label="`Уровень агрессии ${r.aggressionLevel}`"
              >
              ...
              <span
                v-if="r.attendance && r.attendance < 60"
                class="t-flag t-flag-amber"
                title="Низкая посещаемость"
                role="img"
                aria-label="Низкая посещаемость"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" aria-hidden="true">
                  <circle cx="12" cy="12" r="9"/><path d="M12 7v5M12 17h.01"/>
                </svg>
              </span>
            </div>
            ...
            <a
              ...
              :aria-label="`Открыть карточку: ${r.fullName}`"
            ></a>

            <!-- Card body: avatar + info -->
            <div class="t-rcard-body">
              <div class="t-rcard-avatar" :class="`a${(idx % 6) + 1}`" aria-hidden="true">{{ initials(r.fullName) }}</div>
              <div class="t-rcard-main">
                <div class="t-rcard-name" :id="`rc-name-${r.id}`">{{ r.fullName }}</div>
                <div class="t-rcard-meta">
                  <template v-if="r.age">{{ r.age }} лет</template>
                  <template v-if="r.age && curatorName(r)"> · </template>
                ...
                <div class="t-rcard-tags">
                  ...
                  <span v-if="r.group?.name || r.groupName" class="t-tag t-tag-sage">{{ r.group?.name || r.groupName }}</span>
                  <span v-if="r.completionStatus === 'completed'" class="t-tag t-tag-neutral">Завершил</span>
                  <span v-if="r.showAggression && r.aggressionLevel" class="t-tag t-tag-rose" :title="r.aggressionNote">
                    Агрессия {{ r.aggressionLevel }}/5
                  </span>
                </div>
              </div>
            </div>
            ...
            <div class="t-rcard-footer">
              <div class="t-attendance-block">
                <span class="t-att-percent">{{ r.attendance || 0 }}<span class="t-att-sign">%</span></span>
                <div>
                  <div class="t-att-label">Посещаемость</div>
                  <div class="t-att-bar" role="img" :aria-label="`Посещаемость ${r.attendance || 0} процентов`">
                    <div
                      class="t-att-bar-fill"
                      :class="attBarClass(r.attendance)"
                      :style="{ width: (r.attendance || 0) + '%' }"
                    ></div>
                  </div>
                </div>
              </div>

              <!-- Actions (z-index above rcard-link) -->
              <div class="t-rcard-actions" @click.stop style="position:relative;z-index:2">
                <button class="t-action-btn" :aria-label="`Редактировать ${r.fullName}`" @click="editRecipient(r)">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <div v-else class="t-list-table-wrap">
          <div class="t-list-table" role="table" aria-label="Список реабилитантов">
            ...
            <div class="t-tr t-thead" role="row">
              ...
              <div role="columnheader">Посещаемость</div>
            ...
            >
              ...
              <div class="t-name-cell" role="cell" @click="openDetails(r.id)" style="cursor:pointer">
                <div class="t-mini-avatar" :class="`a${(idx % 6) + 1}`" aria-hidden="true">{{ initials(r.fullName) }}</div>
                <div class="t-name-main">
                  <div class="t-name-text">
                    {{ r.fullName }}
                    <span
                      v-if="r.showAggression && r.aggressionLevel >= 3"
                      class="t-tr-flag t-tr-flag-rose"
                      :title="r.aggressionNote"
                      role="img"
                      aria-label="Агрессия"
                    >
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" aria-hidden="true">
                  ...
                  <div class="t-name-sub">{{ r.diagnosis || '—' }} · {{ r.age ? r.age + ' лет' : '' }}</div>
                </div>
              </div>

              <div class="t-tr-cell t-col-group" role="cell">{{ r.group?.name || r.groupName || '—' }}</div>
              <div class="t-tr-cell t-col-curator" role="cell">{{ curatorName(r) || '—' }}</div>

              <!-- Attendance cell -->
              <div class="t-tr-attendance" role="cell">
                <span class="t-att-num">{{ r.attendance || 0 }}%</span>
                <div class="t-att-bar-sm">
                  <div
                    class="t-att-bar-sm-fill"
                    :class="attBarClass(r.attendance)"
                    :style="{ width: (r.attendance || 0) + '%' }"
                  ></div>
                </div>
        ...
        <!-- Edit Modal (teacher view) -->
        <Modal v-if="showModal && editId" :title="modalTitle" @close="closeModal">
          <form @submit.prevent="saveRecipient" class="recipient-form">
            <div class="form-grid">
              <div class="form-group"><label>ФИО</label><input v-model="form.fullName" required /></div>
              <div class="form-group"><label>Возраст</label><input v-model.number="form.age" type="number" /></div>
              ...
              <div class="form-group"><label>Контакты (телефон, email)</label><input v-model="form.contacts" /></div>
              <div class="form-group"><label>Законный представитель</label><input v-model="form.legalRepresentative" /></div>
              <div class="form-group"><label>Уровень агрессии (1–5)</label><input v-model.number="form.aggressionLevel" type="number" min="1" max="5" /></div>
              <div class="form-group checkbox-group">
                <label><input type="checkbox" v-model="form.showAggression" /> Показывать значок агрессии</label>
              </div>
              <div class="form-group full-width"><label>Описание угрозы</label><textarea v-model="form.aggressionNote" rows="2"></textarea></div>
              <div class="form-group">
                <label>Статус реабилитации</label>
                <select v-model="form.completionStatus">
                  <option value="in_progress">В процессе</option>
                  <option value="completed">Прошел</option>
                </select>
              </div>
            </div>
    ...
    <!-- ═══════════════════════════════════════════════
         ADMIN / OTHER ROLES VIEW  (существующий дизайн)
    ═══════════════════════════════════════════════ -->
    <template v-else>
      <div class="toolbar">
        <div class="search">
        ...
        <div v-for="r in recipients" :key="r.id" class="item-card" @click="openDetails(r.id)">
          ...
          <div class="card-left">
            <div class="avatar-wrapper">
              ...
              <div v-if="r.showAggression" class="aggression-badge" :style="{ backgroundColor: aggressionColor(r.aggressionLevel) }">
                {{ r.aggressionLevel || 1 }}
                <span class="aggression-tooltip">{{ (r.aggressionNote && r.aggressionNote.trim()) ? r.aggressionNote : 'Нет описания' }}</span>
              </div>
            </div>
            <div class="card-info">
              <div class="name">{{ r.fullName }}</div>
              <div class="sub">Куратор: {{ r.group?.curator?.fullName || r.curator || '—' }}</div>
              <div class="tags">
          ...
          <div class="card-right">
            <div class="widget-status" :title="r.completionStatus === 'completed' ? 'Реабилитация пройдена' : 'В процессе'">
              <svg v-if="r.completionStatus === 'completed'" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M20 6L9 17l-5-5" stroke-width="2" /></svg>
              <svg v-else width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="10" stroke-width="1.5" /><path d="M12 6v6l4 2" stroke-width="1.5" /></svg>
              <span class="widget-value">{{ r.completionStatus === 'completed' ? 'Прошел' : 'В процессе' }}</span>
            </div>
            <div class="widget-attendance">
              ...
              <span class="widget-value">{{ r.attendance || 0 }}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal v-if="showModal && editId" :title="modalTitle" @close="closeModal">
        <form @submit.prevent="saveRecipient" class="recipient-form">
          <div class="form-grid">
            ...
            <div class="form-group"><label>Законный представитель</label><input v-model="form.legalRepresentative" /></div>
            <div class="form-group"><label>Уровень агрессии (1–5)</label><input v-model.number="form.aggressionLevel" type="number" min="1" max="5" /></div>
            <div class="form-group checkbox-group"><label><input type="checkbox" v-model="form.showAggression" /> Показывать значок агрессии</label></div>
            <div class="form-group full-width"><label>Описание угрозы</label><textarea v-model="form.aggressionNote" rows="2"></textarea></div>
            <div class="form-group"><label>Статус реабилитации</label><select v-model="form.completionStatus"><option value="in_progress">В процессе</option><option value="completed">Прошел</option></select></div>
          </div>
          <div class="form-actions">
            <button type="button" class="btn-secondary" @click="closeModal">Отмена</button>
            <button type="submit" class="btn-primary">Сохранить</button>
...
const loading      = ref(true);   // true → спиннер сразу, без мигания «пустой» страницы
const error        = ref('');
const showModal    = ref(false);
const modalTitle   = ref('');
const form = ref({
  fullName: '', age: null, diagnosis: '', groupId: null, photo: '',
  contacts: '', legalRepresentative: '',
  aggressionLevel: 1, showAggression: true, aggressionNote: '', completionStatus: 'in_progress'
});
...
const activeFilter = ref('all');   // 'all' | 'attention' | 'in_progress' | 'completed'
const selectedIds  = ref([]);
const searchFocused = ref(false);

// ─── Teacher-view computed ──────────────────────────────────────
const filteredRecipients = computed(() => {
  switch (activeFilter.value) {
    case 'attention':   return recipients.value.filter(r => hasAlert(r));
    case 'in_progress': return recipients.value.filter(r => r.completionStatus === 'in_progress');
    case 'completed':   return recipients.value.filter(r => r.completionStatus === 'completed');
    default:            return recipients.value;
  }
});
const attentionCount  = computed(() => recipients.value.filter(r => hasAlert(r)).length);
const inProgressCount = computed(() => recipients.value.filter(r => r.completionStatus === 'in_progress').length);
const completedCount  = computed(() => recipients.value.filter(r => r.completionStatus === 'completed').length);
...
const curatorName = (r) => r.group?.curator?.fullName || r.curator || '';
const attBarClass = (att) => {
  const n = att || 0;
  if (n >= 80) return '';
  if (n >= 60) return 'warn';
  return 'bad';
...
const hasAlert = (r) =>
  (r.showAggression && r.aggressionLevel >= 3) ||
  (r.attendance !== undefined && r.attendance !== null && r.attendance < 60);

// ─── Teacher filter/select methods ──────────────────────────────
...
// ─── Old admin view helpers ──────────────────────────────────────
const viewEvents      = (r) => { console.log('События', r.fullName);     closeDropdown(); };
const viewAttendance  = (r) => { console.log('Посещаемость', r.fullName); closeDropdown(); };
const viewPerformance = (r) => { console.log('Успеваемость', r.fullName); closeDropdown(); };
const aggressionColor = (level) => {
  switch (level) {
    case 1: return '#6c757d'; case 2: return '#b0a27b'; case 3: return '#ffc107';
    case 4: return '#fd7e14'; case 5: return '#dc3545'; default: return '#6c757d';
  }
};
...
const editRecipient = (r) => {
  ...
  form.value = {
    ...
    aggressionLevel:     r.aggressionLevel || 1,
    showAggression:      r.showAggression !== undefined ? r.showAggression : true,
    aggressionNote:      r.aggressionNote || '',
    completionStatus:    r.completionStatus || 'in_progress'
  };
...
<style scoped>
/* ══════════════════════════════════════════════════════
   DESIGN TOKENS — teacher view wrapper
══════════════════════════════════════════════════════ */
.erp-r-teacher {
  /* Type */
...
/* ══════════════════════════════════════════════════════
   ADMIN VIEW — existing styles (unchanged)
══════════════════════════════════════════════════════ */
